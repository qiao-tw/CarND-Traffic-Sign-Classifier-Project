...
Learning rate is 0.0008. I chose this number because 0.001 (default learning rate by copy/paste from LeNet Lab.) seems working fine but finally damping around accuracy 0.91~0.94. To make it converge to a higher accuracy, lowering down learning rate seems the right way to go.

Within each epoch, the accuracy in validation are printed out. The accuracy is continually increasing in most of the epochs. At the last epoch(EPOCH 150), validation accuracy reach around 0.94, which is a satisfactory result.
